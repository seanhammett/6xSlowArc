...
<div class="card" id="audiocard">
 ...
 <div id="aubody" style="display:none">
  ...
  <div class="wrow"><label class="chk" title="Nudges the sequence's clock by a few ms at a time to match the audio. Turn on in one browser only."><input type="checkbox" id="aufollow" onchange="auFollow()"> Keep the lights in step with the audio</label></div>
  <div id="autrack"></div>
  <div id="austat"></div>
 </div>
</div>
...
function auSync(){
  ...
  const wrapped=AU.lastLoop>=0&&loopN!==AU.lastLoop&&!AU.el.loop;
...
function auRender(){
  ...
  if(!AU.url)s='choose the track file on this computer';
    ...
    +(AU.follow?' · lights follow the audio':' · free-running');
